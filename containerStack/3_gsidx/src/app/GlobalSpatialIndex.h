...
#ifndef LSST_QSERV_GSIDX_GLOBALSPATIALINDEX_H
#define LSST_QSERV_GSIDX_GLOBALSPATIALINDEX_H
...
/**
 * Class GlobalSpatialIndex is an interface for getting chunk and subchunk location of object by its
 * objectId.
 *
 ...
 */
class GlobalSpatialIndex {
public:

    // Constructor that takes a running io_context allows the caller to establish the threading policy.
    GlobalSpatialIndex(std::string const& name, boost::asio::io_context& io_context);

    enum Err { SUCCESS, FAIL };

    std::vector<std::string> scan(std::string const& pattern);
     ...
     * @param onComplete a function to call once the value has been set in the gsidx data store.
     */
    ...
    GlobalSpatialIndex(GlobalSpatialIndex const&)=delete;
    GlobalSpatialIndex& operator=(GlobalSpatialIndex const&)=delete;

    ~GlobalSpatialIndex() = default;
...
#endif // LSST_QSERV_GSIDX_GLOBALSPATIALINDEX_H
